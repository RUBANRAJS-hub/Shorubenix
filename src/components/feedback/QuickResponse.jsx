...
export default function QuickResponse() {
  ...
  const defaultMsg = encodeURIComponent(
    'Hi Shorubenix Info Technology! I am looking for a quick response regarding software development or academic services. Could you please help me?'
  )

  const handleWhatsAppClick = () => {
    window.open(`https://wa.me/${COMPANY_INFO.whatsappNumber}?text=${defaultMsg}`, '_blank', 'noopener,noreferrer')
    setIsOpen(false)
  }
      ...
      {showTooltip && !isOpen && (
        <div className="quick-response-bubble animate-fadeUp">
          ...
          <div className="quick-response-bubble__content" onClick={() => setIsOpen(true)}>
            ...
            <p className="quick-response-bubble__text">
              🚀 Need a custom app, cloud setup, or academic R&D? Get a <strong>Quick Response</strong> in 5 mins!
            </p>
          </div>
        </div>
      )}
      ...
      {isOpen && (
        <div className="quick-response-panel animate-fadeUp">
          ...
          <div className="quick-response-panel__body">
            <p className="quick-response-panel__intro">
              👋 Hello! How can we assist you today? Select a contact method below for instant connection.
            </p>
